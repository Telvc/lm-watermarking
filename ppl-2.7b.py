...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", type=str, default="Qwen/Qwen2.5-Omni-7B",#"facebook/opt-1.3b",
...
def construct_similarity_matrix_cos(vocab_list: list[str], indices: list[int],
...
def evaluate_watermarking(truncated_texts, model, tokenizer, args, args_cos=None):
    ...
    for prompt in tqdm(truncated_texts, desc="Evaluating prompts"):
        ...
        if (valid_length(decoded_wm, decoded_nw, tokenizer) and
            ...
            print(
                " Default Method:    avg ppl (wm) = {0:.2f}, avg ppl (nw) = {1:.2f}, AUC = {2:.3f}, avg z (wm) = {3:.2f}, avg z (nw) = {4:.2f}".format(
                    curr_avg_ppl_wm_default, curr_avg_ppl_nw_default, curr_auc_default, curr_avg_z_wm_default,
                    curr_avg_z_nw_default))
            print(
                " KGW Method:        avg ppl (wm) = {0:.2f}, avg ppl (nw) = {1:.2f}, AUC = {2:.3f}, avg z (wm) = {3:.2f}, avg z (nw) = {4:.2f}".format(
                    curr_avg_ppl_wm_kgw, curr_avg_ppl_nw_kgw, curr_auc_kgw, curr_avg_z_wm_kgw, curr_avg_z_nw_kgw))
            print(
                " Cosine-based Method: avg ppl (wm) = {0:.2f}, avg ppl (nw) = {1:.2f}, AUC = {2:.3f}, avg z (wm) = {3:.2f}, avg z (nw) = {4:.2f}\n".format(
